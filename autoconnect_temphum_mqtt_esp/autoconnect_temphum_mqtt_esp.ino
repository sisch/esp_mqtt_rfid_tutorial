...
char mqtt_user[40];
char mqtt_pw[64];
char mqtt_topic_temp[128] = "sensors/office1/temp";
char mqtt_topic_humi[128] = "sensors/office1/humi";
  ...
  if (SPIFFS.begin()) {
    ...
    if (SPIFFS.exists("/config.json")) {
      ...
      if (configFile) {
        ...
        strcpy(mqtt_topic_temp, jsondoc["mqtt_topic_temp"]);
        strcpy(mqtt_topic_humi, jsondoc["mqtt_topic_humi"]);
...
void save_json() {
  ...
  jsondoc["mqtt_topic_temp"] = mqtt_topic_temp;
  jsondoc["mqtt_topic_humi"] = mqtt_topic_humi;
...
void setup()
{
  ...
  WiFiManagerParameter custom_mqtt_topic_temp("topicT", "mqtt topic temp", mqtt_topic_temp, 128);
  WiFiManagerParameter custom_mqtt_topic_humi("topicH", "mqtt topic humi", mqtt_topic_humi, 128);
  ...
  wifimanager.addParameter(&custom_mqtt_topic_humi);
  ...
  strcpy(mqtt_topic_humi, custom_mqtt_topic_humi.getValue());
